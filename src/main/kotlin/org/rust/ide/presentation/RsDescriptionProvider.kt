...
class RsDescriptionProvider : ElementDescriptionProvider {
    ...
    private fun refactoringDescription(element: PsiElement, includeParent: Boolean): String {
        ...
        val elementName = defaultDescription(element) ?: return ""

        val name = if (includeParent) {
            val parent = element.parent
            when {
                element is RsMod -> element.qualifiedName
                parent is RsMod -> parent.qualifiedName?.let { "$it::$elementName" }
                else -> elementName
            } ?: elementName
        } else {
            elementName
