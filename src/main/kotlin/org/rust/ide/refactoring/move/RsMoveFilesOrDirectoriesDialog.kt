...
class RsMoveFilesOrDirectoriesDialog(
    ...
    fun doPerformMove(targetDirectory: PsiDirectory, searchForReferences: Boolean, doneCallback: Runnable) {
        ...
        val movedFile = elementsToMove[0] as RsFile
        // we check that movedFile.crateRoot is not null in RsMoveFilesOrDirectoriesHandler::canMove
        val newParentMod = targetDirectory.getOwningMod(movedFile.crateRoot!!)
        when {
            newParentMod == null -> {
                if (shouldMoveIfNoNewParentMod()) {
                    runDefaultProcessor()
                }
            }
    ...
    private fun shouldMoveIfNoNewParentMod(): Boolean {
